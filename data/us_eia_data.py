...
easily generate the URLs for the GET requests by selecting the appropriate table at https://www.eia.gov/opendata/browser/. A major part if
the proprcessing is done here.
...
def retrieve_data(path="", download=True):
    ...
    for key in api_urls:
        ...
        # this is necessary since we need to retrieve the data for the other two variables for each date individually, starting from first day
        if key != "average_price":
            ...
            for day in days:
                ...
                if int(str(day)[:4]) < 2018:
                    ...
                    # just append an "NaN"
                    continue
                api_url = api_urls[key].replace("xxxx", api_key) + "&start=" + str(day) + "&end=" + str(day)
                print(api_url)
                ...
                # for every day, we need to find all data points
            ...
            for day in days:
                ...
                if match_found and not str(data[i]["price"]) == "null":
                    historic_data[key].append(data[i]["price"])
                else:
                    # otherwise a "NaN" is added when the date does not exist in the data or when the data is "null"
                    historic_data[key].append("NaN")
                        
    for key in api_urls:
        print("For the " + key + " data there are " + str(historic_data[key].count("NaN")) + " NaN values.")
